...
int nvgCreateImageFromUrl(NVGcontext* ctx, const char* filename, int imageFlags)
{
    int w, h, n, image;
        unsigned char* img;
        stbi_set_unpremultiply_on_load(1);
        stbi_convert_iphone_png_to_rgb(1);
        img = stbi_load_fromUrl(filename, &w, &h, &n, 4);
        if (img == NULL) {
    //        printf("Failed to load %s - %s\n", filename, stbi_failure_reason());
            return 0;
        }
...
//        printf("process: %s", iter.str == NULL ? "…" : iter.str);
		...
		if (type == NVG_NEWLINE) {
			...
			if (rowStart == NULL) {
				...
				if (((ptype == NVG_CHAR || ptype == NVG_CJK_CHAR || ptype == NVG_SPACE) && type == NVG_SPACE) || type == NVG_CJK_CHAR) {
					breakEnd = iter.str;
					breakWidth = rowWidth;
					breakMaxX = rowMaxX;
				}
